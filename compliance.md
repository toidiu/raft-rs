...
- [x] If last log index ≥ nextIndex for a follower: send AppendEntries RPC with log entries starting at nextIndex
  - [x] If successful: update nextIndex and matchIndex for follower (§5.3)
  - [x] If AppendEntries fails because of log inconsistency: decrement nextIndex and retry (§5.3)
- [x] If there exists an N such that N > commitIndex, a majority of matchIndex[i] ≥ N, and log[N].term == currentTerm: set commitIndex = N (§5.3, §5.4).
...
- [x] success: true if follower contained entry matching prevLogIndex and prevLogTerm
